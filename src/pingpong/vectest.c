...
//    int printMap;    /* Print the mapping of processes to the topology */
    ...
    int stride;      /* Stride for vector type */
    ...
    int checksync;   /* value 1 if serialize check to improve readability of
			output */
...
int main(int argc, char **argv)
{
    ...
    /* Create variable resized datatype */
    MPI_Type_create_resized(MPI_DOUBLE, 0, options.stride*sizeof(double),
			    &vectype);
    MPI_Type_commit(&vectype);

    /* TODO: Need options for: recv with type as well as send (will need
       a different check routine); MPI_Type_vector as an option (but a
       separate type for each msglen) */
    ...
    /* Send buffer is maxlenbytes * stride */
    mosend = BENV_MemAlloc(maxlenbytes*options.stride, options.mtype);
	...
	for (int lidx=0; lidx<options.nMsgSizes; lidx++) {
	    ...
	    BENV_MemInitValue1D(mosend, 2*msglen*options.stride,
				wrank*msglen*options.stride,
				0, msglen*options.stride, 0.0, 1.0);
	    /* Init the receive buffer so we can spot unset entries */
	    BENV_MemInitValue1D(morecv, 2*msglen, wrank*msglen, 0, msglen,
				-1.0, -1.0);
#if 0
	    if (wrank == 1) {
		fprintf(stdout, "Send buffer init as: ");
		printDoubleArray(stdout, mosend->memptr, msglen*options.stride);
	    }
#endif
		...
		if (wrank == srcrank) {
		    MPI_Send(mosend->memptr, msglen, vectype, destrank, iter,
			     MPI_COMM_WORLD);
		    ...
		    MPI_Recv(morecv->memptr, msglen, MPI_DOUBLE, srcrank, iter,
			     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		    MPI_Send(mosend->memptr, msglen, vectype, srcrank, iter,
			     MPI_COMM_WORLD);
		}
	    }
	    t0 = MPI_Wtime() - t0;
...
#if 0
	double quartiles[5];
	fprintf(outfp, "@n\tmin\t1Q\t2Q\t3Q\tmax\n");
	for (int lidx=0; lidx<options.nMsgSizes; lidx++) {
	    BENV_TAFindQuartiles(options.nTrials, 1, ta->data + offset, quartiles);
	    fprintf(outfp, "%d\t%.2e\t%.2e\t%.2e\t%.2e\t%.2e\n",
		    options.msgsizes[lidx], quartiles[0],
...
#if 0
	    {
		double *dd = ta->data + offset;
		printf("For tests for lidx %d\n", lidx);
		for (int j=0; j<options.nTrials; j++) {
		    printf("%.2e\t", dd[j]);
		}
		putchar('\n');
	    }
...
#if 0
	    {
		printf("Found quartiles: ");
		for (int j=0; j<5; j++) printf("%.2e\t", q[5*lidx+j]);
		putchar('\n');
	    }
#endif
...
//    options->mapname       = 0;
	...
	rc = BENV_HwdescArg(argc, argv, &i, "-hw", &options->hwparms);
	...
	if      (strcmp(argv[i], "-v") == 0)  {
	...
	else if (strcmp(argv[i], "-checksync") == 0) {
	    options->checksync = 1;
	}
	    ...
	    i++;
	    if (BENV_ArgGetint(i, argc, argv, &options->stride))
		return 1;
	}
	...
	options->msgsizes = BENV_GetSizesMult(1, 32*1024, 2.0,
					     &options->nMsgSizes);
    if (!options->outName)
	options->outName = strdup("vectest.dat");
    if (options->hwparms.printMap && !options->hwparms.mapname)
	options->hwparms.mapname = strdup("vectest.map");
...
{
    fprintf(stderr, "\
vectest - Test MPI communication from CPU and GPUs\n");
 ...
 -sizes sizearg - Specify the number of doubles to send. May be a range,\n\
                  list, or combination\n\
 -stride n  - Stride for the vector types\n\
	       ...
	       int trial, long msglen)
{
    ...
    double incr = 1.0 * options->stride;
    ...
    if (options->checksync) BENV_SeqBegin(MPI_COMM_WORLD);
    ...
    if (options->checksync) BENV_SeqEnd(MPI_COMM_WORLD);
}
