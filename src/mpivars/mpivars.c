 ...
 *     See COPYRIGHT in top-level directory of MPICH (see mpich.org)
 *
 * This code originally written by William Gropp
 *
 * Updates Copyright (C) University of Illinois 2022
 */
...
/* An MPI implementation is permitted to provide information only on the
   tools variables (control, performance, and events) that are active,
   e.g., if there are no references to File I/O or RMA, those variables
   and event may not be shown. If this value is defined, this test
   adds references to RMA and I/O. If other refrences are needed,
   they can be added in the same way.
*/
#define INIT_MPI_SUBSYSTEMS 1
...
int checkStringLen(const char *label, const char *varname,
		   const char *str, int expectedLen, const char *strName);

/* Variable descriptions are sometimes long - this variable controls
   whether they are printed */
static int showDesc = 1;
/* Accessing the value of some variables sometimes causes a SEGV. This may
   reflect a bug in the MPI implementation. */
static int showValues = 1;

/*D
  mpivars - Print the MPI control and performance variables and events

Synopsis:
  mpiexec -n 1 ./mpivars

Command Line Options:
+ --nodesc - Do not show the descriptions of variables or events
. --novalue - Do not show the values of variables
- -o name - File name for output; stdout by default
...
int main(int argc, char *argv[])
{
	...
	else if (strcmp(argv[i], "--novalues") == 0 ||
	         strcmp(argv[i], "-novalues") == 0) {
	    showValues = 0;
	}
        else {
            if (wrank == 0) {
                fprintf(stderr, "Unrecognized command line argument %s\n", argv[i]);
                fprintf(stderr, "Usage: mpivars [ --nodesc ] [ --novalues ] [ -o fname ]\n");
                MPI_Abort(MPI_COMM_WORLD, 1);
                return 1;
            }
        }
...
#ifdef INIT_MPI_SUBSYSTEMS
    /* Make calls that are likely to load modules that might not be
       loaded by default.  Making these calls may make additional control
       and performance variables visible. */
    MPI_Win_create(&buf, sizeof(double), sizeof(double), MPI_INFO_NULL,
                   MPI_COMM_WORLD, &win);
    MPI_File_open(MPI_COMM_WORLD, "mpit-test.dat",
                  MPI_MODE_CREATE | MPI_MODE_RDONLY, MPI_INFO_NULL, &fp);
#endif

    PrintControlVars(outfd);
    fprintf(outfd, "\n");
    PrintPerfVars(outfd);
    fprintf(outfd, "\n");
    PrintCategories(outfd);
#if MPI_VERSION >= 4
    fprintf(outfd, "\n");
    PrintEvents(outfd);
#endif
    ...
    MPI_Win_free(&win);
    MPI_File_close(&fp);
    MPI_File_delete("mpit-test.dat", MPI_INFO_NULL);
#endif
...
int PrintControlVars(FILE * fp)
{
    ...
    for (i = 0; i < num_cvar; i++) {
        ...
        checkStringLen("cvar_get_info", name, desc, descLen, "descLen");
...
int PrintPerfVars(FILE * fp)
{
    ...
    for (i = 0; i < numPvar; i++) {
        ...
        checkStringLen("pvar_get_info", NULL, name, nameLen, "nameLen");
	...
	/* Getting the current Performance variable value requires
	   identifying the object type and providing that to the
	   access routine */
	/* Note that getting a pvar value requires a valid object, which
	   we may not know */
	if (0 & showValues) {
...
int PrintCategories(FILE * fp)
{
    ...
    for (i = 0; i < numCat; i++) {
        ...
        MPI_T_category_get_info(i, name, &nameLen, desc, &descLen, &numCvars,
                                &numPvars, &numSubcat);
        /* Check on correct output */
        checkStringLen("category_get_info", NULL, name, nameLen, "nameLen");
	...
	/* FIXME: Recursively handle subcategories */
        if (numSubcat > 0) {
...
#if MPI_VERSION >= 4
/* Print all of the MPI_T events, along with their major properties */
int PrintEvents(FILE * fp)
{
    int i, numEvents, nameLen, descLen, verbosity;
    int binding;
    char name[MAX_NAME_LEN], desc[MAX_DESC_LEN];
    MPI_T_enum enumtype;

    MPI_T_event_get_num(&numEvents);
	...
	int numelements = 0;
        nameLen = sizeof(name);
        descLen = sizeof(desc);
        MPI_T_event_get_info(i, name, &nameLen, &verbosity, NULL, NULL,
			     &numelements, &enumtype, NULL,
			     desc, &descLen, &binding);
        /* Check for correct return values */
        checkStringLen("event_get_info", NULL, name, nameLen, "nameLen");
...
/*
 * See Table 15.3 in MPI 4.0 for the *complete* list of valid MPI
 * datatypes that can be used in the MPI tool interface.
 *
 * Invalid MPI Datatypes used by some MPI implementation
 *   MPI_C_BOOL
 */
const char *mpit_validDtypeStr(MPI_Datatype datatype)
{
...
int getCvarValueAsStr(int idx, MPI_Datatype datatype, char varValue[], int varValueLen)
{
    ...
    if (varValueLen > 0)
	varValue[0] = 0;   /* Just in case */

    MPI_T_cvar_handle_alloc(idx, NULL, &chandle, &count);
    if (count == 1 ||
	(datatype == MPI_CHAR && count < varValueLen && count > 0)) {
        if (MPI_INT == datatype) {
	    ...
	    /* Invalid type (see Table 15.3 in MPI 4.0). We may be able to
	       show the value even so */
	    if (MPI_C_BOOL == datatype) {
		MPI_T_cvar_read(chandle, &ival);
		varValue[0] = ival ? 'T' : 'F';
		varValue[1] = 0;
		hasValue = 1;
		...
		/* Attempt to report the erroneous type */
		char *tname = (char *)malloc(MPI_MAX_OBJECT_NAME+1);
		int  tlen=0;

		MPI_Type_get_name(datatype, tname, &tlen);
		if (tlen > 0 && tname[0]) {
		    fprintf(stderr, "cvar_read_val with improper datatype %s\n",
			    tname);
		}
		else {
		    fprintf(stderr, "cvar_read val with unnamed improper datatype %ld\n", (long)datatype);
		}
		fflush(stderr);
		free(tname);
	    }
...
/* Read a performance variable value and return the value as a null terminated
   ...
   Note: This code is not quite correct. The routine MPI_T_pvar_handle_alloc
   needs a valid object in the third argument. MPICH allows a null object but
   Open MPI does not, and the standard does not guarantee that NULL may be
                      ...
                      char varValue[], int varValueLen)
{
    ...
    /* FIXME: cannot use NULL as the input object handle */
...
/* Several of the MPI_T routines return a string length as well as the string.
   ...
   got this wrong).
   varname should be non-null only if the string is *not* the name, e.g.,
   the description */
int checkStringLen(const char *label, const char *varname,
		   const char *str, int expectedLen, const char *strName)
{
    ...
    if (expectedLen != actLen) {
        fprintf(stderr,
                "Warning: In %s(%s), incorrect return value for %s = %d, should = %d\n", label, varname ? varname : str, strName, expectedLen, actLen);
        return 1;
    }
    return 0;
}
