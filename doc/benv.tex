...
In addition, the configuration may represent all of the resources, or
only those available to the program.
Which it is depends on the routines available to acquire information
as well as other implementation choices.
...
Further, Base Env represents this with two separate hierarchies: one
based solely on the hardware and one built around MPI processes and
communicators.
...
\input{hwdesc.tikz}
\end{center}
\caption{An example configuration with one node, one socket, two NUMA
  regions, each with two cores. The circles denote MPI ranks, relative
to the communicator in the smallest containing box. Circles along the
same horizontal line are in the same MPI process.}
\label{fig:hwdesc}
\end{figure}
...
ToDo: Add examples about forcing a particular configuration and/or
assignment, e.g., with \texttt{-numnodes 4 -numsockets 2 -forcedebug}

ToDo: Summarize the test programs, as they are also examples
